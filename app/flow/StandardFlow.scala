...
class StandardFlow(flowData: FlowData) extends Flow:
  ...
  private def handleProcessTransaction(submittedData: SubmittedData): SpcBehaviour = behave {
    ...
    case processTransaction: ProcessTransaction =>
      ...
      val interimResponses = flowData.displayMessagesValidation.map { case (interactionEvents, interactionPrompt) =>
        val interactionNode             =
  ...
  private def handleUpdatePaymentEnhancedResponse(submittedData: SubmittedData): SpcBehaviour = behave {
    case updatePaymentEnhancedResponse: UpdatePaymentEnhancedResponse =>
      ...
      val interimResponses = flowData.displayMessagesAuthentication.map { case (interactionEvents, interactionPrompt) =>
        val interactionNode             =
  ...
  private def handlePosPrintReceiptResponse(
    ...
    (
      ...
      handlePosPrintReceiptResponseWithPtr(
        submittedData,
        totalAmount,
        finalAmount,
        merchantReceiptNode,
