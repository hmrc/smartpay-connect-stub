...
class NoReceiptFlow(flowData: FlowDataNoReceipt) extends Flow:
  ...
  private def handleSubmitPayment: SpcBehaviour = behave { case submitPayment: SubmitPayment =>
  ...
  private def handleProcessTransaction(submittedData: SubmittedData): SpcBehaviour = behave {
    case processTransaction: ProcessTransaction =>
      ...
      val interimMessages = flowData.displayMessagesValidation.map { case (interactionEvents, interactionPrompt) =>
        val interactionNode             =
      ...
      (
        interimMessages :+ updatePaymentEnhanced,
  ...
  private def handleUpdatePaymentEnhancedResponse(submittedData: SubmittedData): SpcBehaviour = behave {
    case updatePaymentEnhancedResponse: UpdatePaymentEnhancedResponse =>
      ...
      val interimMessages = flowData.displayMessagesAuthentication.map { case (interactionEvents, interactionPrompt) =>
        val interactionNode             =
