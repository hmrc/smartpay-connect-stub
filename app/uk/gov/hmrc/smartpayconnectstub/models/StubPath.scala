...
object StubPaths extends Enum[StubPath] {
  ...
  final case object SuccessKeyed extends StubPath { val value =  "success_keyed_test" ; val description = "Card keyed success path with surcharge accepted. For test only. Not supported on production"}
  final case object CardDeclinedKeyed extends StubPath { val value =  "card_declined_keyed_test" ; val description = "Card keyed  path with card  declined by spc. For test only. Not supported on production"}
