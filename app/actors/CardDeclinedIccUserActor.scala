...
//*/
//
//object CardDeclinedIccUserActor {
//  def props():Props = Props(new CardDeclinedIccUserActor())
//}
//
//
//class CardDeclinedIccUserActor extends Actor {
//  import SpcParentActor._
//  var state:SpcState = ScpState(AmountInPence.zero, StubUtil.VisaCredit, None, Country.Uk, Currency.Gbp, TransactionSources.Icc)
...
//      val posDisplayMessageInProgress = ErrorMessage(HeaderNode(),unexpected.messageNode, ErrorsNode(Seq(StubUtil.incorrectMessageFlowErrorNode)), SuccessResult)
...
//      val visaCardNode = CardNode(state.paymentCard.currency, state.paymentCard.country, state.paymentCard.endDate, state.paymentCard.startDate, state.paymentCard.pan, state.paymentCard.cardSchema)
...
//
//      val posPrintReceipt = PosPrintReceipt(HeaderNode(), updatePaymentEnhancedResponse.messageNode, StubUtil.customerReceipt, SuccessResult,ErrorsNode(Seq.empty))
...
//      val ptrTransactionNode = PtrTransactionNode(amountNode,Some(TransactionActions.AuthorizeAndSettle), Some(TransactionTypes.Purchase), Some(state.source), Some(TransactionCustomers.Present),Some(StubUtil.transactionReference))
//      val visaPtrCardNode = PtrCardNode(state.paymentCard.currency, state.paymentCard.country, state.paymentCard.endDate, state.paymentCard.startDate, state.paymentCard.pan, state.paymentCard.cardSchema)
//
//      val processTransactionResponse = ProcessTransactionResponse(HeaderNode(), messageNode = cancelTransaction.messageNode, ptrTransactionNode = ptrTransactionNode, ptrCardNode = visaPtrCardNode, result = TranResults.SuccessResult, paymentResult = PaymentResults.cancelled, receiptNodeCustomerO = None, receiptNodeMerchantO = None, errorsNode = ErrorsNode(Seq.empty))
...
//      val visaPtrCardNode = PtrCardNode(state.paymentCard.currency, state.paymentCard.country, state.paymentCard.endDate, state.paymentCard.startDate, state.paymentCard.pan, state.paymentCard.cardSchema)
//
//      val processTransactionResponse = ProcessTransactionResponse(headerNode = HeaderNode(), messageNode = posPrintReceiptResponse.messageNode, ptrTransactionNode = ptrTransactionNode, ptrCardNode = visaPtrCardNode, result = TranResults.FailureResult, paymentResult = PaymentResults.declined, receiptNodeCustomerO = Some(StubUtil.customerReceipt), receiptNodeMerchantO = Some(StubUtil.merchantSignatureReceipt), errorsNode = ErrorsNode(Seq(ErrorNode("100007","Validation of card has failed"))))
//      sendScpReplyMessage(out,processTransactionResponse)
//
//      context.become(handleFinalise orElse handleScpMessages)
//      context.stop(session)
...
//  }
//
//  lazy val logger:Logger = Logger(CardDeclinedIccUserActor.getClass)
//
