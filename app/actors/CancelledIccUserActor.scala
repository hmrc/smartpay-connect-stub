...
//
//object CancelledIccUserActor {
//  def props():Props = Props(new CancelledIccUserActor())
//}
//
//class CancelledIccUserActor extends Actor {
...
//      val visaPtrCardNode = PtrCardNode(state.paymentCard.currency, state.paymentCard.country, state.paymentCard.endDate, state.paymentCard.startDate, state.paymentCard.pan, state.paymentCard.cardSchema)
//
//      val processTransactionResponse = ProcessTransactionResponse(headerNode = HeaderNode(), messageNode = posPrintReceiptResponse.messageNode, ptrTransactionNode = ptrTransactionNode, ptrCardNode = visaPtrCardNode, result = TranResults.SuccessResult, paymentResult = PaymentResults.cancelled, receiptNodeCustomerO = Some(StubUtil.customerReceipt), receiptNodeMerchantO = Some(StubUtil.merchantSignatureReceipt), errorsNode = ErrorsNode(Seq.empty))
//      sendScpReplyMessage(out,processTransactionResponse)
//
//      context.become(handleFinalise orElse handleScpMessages)
//      context.stop(session)
...
//  }
//
//  lazy val logger:Logger = Logger(CancelledIccUserActor.getClass)
//
