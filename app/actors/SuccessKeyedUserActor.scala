...
//      val errorMessage = ErrorMessage(HeaderNode(),unexpected.messageNode, ErrorsNode(Seq(StubUtil.incorrectMessageFlowErrorNode)), SuccessResult)
...
//
//      val posPrintReceipt = PosPrintReceipt(HeaderNode(), updatePaymentEnhancedResponse.messageNode, StubUtil.merchantSignatureReceipt, SuccessResult,ErrorsNode(Seq.empty))
...
//
//      val processTransactionResponse = ProcessTransactionResponse(HeaderNode(), cancelTransaction.messageNode, ptrTransactionNode, visaPtrCardNode, TranResults.SuccessResult, PaymentResults.cancelled, None, None, ErrorsNode(Seq.empty))
...
//
//      val posPrintReceipt = PosPrintReceipt(HeaderNode(), completeTransaction.messageNode, StubUtil.merchantSignatureReceipt, SuccessResult,ErrorsNode(Seq.empty))
//      sendScpReplyMessage(out,posPrintReceipt)
//
...
//      val visaPtrCardNode = PtrCardNode(state.paymentCard.currency, state.paymentCard.country, state.paymentCard.endDate, state.paymentCard.startDate, state.paymentCard.pan, state.paymentCard.cardSchema)
//
//      val processTransactionResponse = ProcessTransactionResponse(HeaderNode(), posPrintReceiptResponse.messageNode, ptrTransactionNode, visaPtrCardNode, TranResults.SuccessResult, PaymentResults.OnlineResult, Some(StubUtil.customerReceipt), Some(StubUtil.merchantSignatureReceipt), ErrorsNode(Seq.empty))
//      sendScpReplyMessage(out,processTransactionResponse)
//
//      context.become(handleFinalise orElse handleScpMessages)
//      context.stop(session)
