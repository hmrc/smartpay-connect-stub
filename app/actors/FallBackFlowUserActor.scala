...
object FallBackFlowUserActor {
  def props(spcFlow: SpcFlow): Props = Props(new FallBackFlowUserActor(spcFlow))
}

class FallBackFlowUserActor(spcFlow: SpcFlow) extends Actor {
  ...
  lazy val logger: Logger = Logger(FallBackFlowUserActor.getClass)
