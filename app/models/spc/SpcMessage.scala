...
/**
 * Smart Pay Connect (Spc) messages, both sent and received.
 */
sealed trait SpcMessage

/**
 * Smart Pay Connect (Spc) Request Message.
 * Those messages are
 * sent from face-to-face-frontend
 * and received at smartpay-connect.
 */
sealed trait SpcRequestMessage extends SpcMessage

object SpcRequestMessage {
  @SuppressWarnings(Array("org.wartremover.warts.Any"))
  implicit val format: Format[SpcRequestMessage] = derived.oformat[SpcRequestMessage]()
}

/**
 * Smart Pay Connect (Spc) Response Message.
 * Those messages are
 * sent from smartpay-connect
 * and received at face-to-face-frontend.
 */
sealed trait SpcResponseMessage extends SpcMessage

final case class PedLogOn(messageNode: MessageNode) extends SpcRequestMessage
...
//icc, keyed
final case class SubmitPayment(messageNode: MessageNode, transactionNode: TransactionNode) extends SpcRequestMessage
...
final case class ProcessTransaction(messageNode: MessageNode) extends SpcRequestMessage
...
final case class Finalise(messageNode: MessageNode) extends SpcRequestMessage
...
final case class PedLogOff(messageNode: MessageNode) extends SpcRequestMessage
...
final case class CancelTransaction(messageNode: MessageNode) extends SpcRequestMessage
...
final case class GetTransactionDetails(messageNode: MessageNode) extends SpcRequestMessage

sealed trait TransactionResponse {
  def messageNode: MessageNode
  def ptrTransactionNode: PtrTransactionNode
  def ptrCardNode: PtrCardNode
  def result: MessageResult
  def paymentResult: PaymentResult
  def merchantNumber: MerchantNumber
  def receiptNodeCustomer: Option[ReceiptNode]
  def errorsNode: ErrorsNode
}
...
final case class ProcessTransactionResponse(
                                             ...
                                             receiptNodeCustomer: Option[ReceiptNode],
                                             errorsNode:          ErrorsNode
                                           ) extends TransactionResponse with SpcResponseMessage
...
//    spc.ErrorMessage(headerNode, messageNode, errorsNode, SuccessResult)
//  }
//
